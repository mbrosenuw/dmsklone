...
DMS = Model(d1, d2, F, Fprime, 0*Qx, 0*Qz, nistconsts, nistconsts, V3, V6, V3p, V3m, chop, mu, jmin, jmax, T, lims, width, shift,
                stats=[1, 1, 1, 1])
DMS.newcalcspectrum(save = True, name = '6_quanta_J_0_40_no_coriolis_NS')
...
# DMS = Model(d1, d2, F, Fprime, 0*Qx, 0*Qz, nistconsts, nistconsts, V3, V6, V3p, V3m, chop, mu, jmin, jmax, T, lims, width, shift,
#                 stats=[1, 1, 1, 1])
DMS.newcalcspectrum(save = True, name = '6_quanta_J_0_40_full_NS')
...
plt.tight_layout()
plt.savefig('fullspectrum_J_40_NS.png')
plt.close()
np.savez('J_0_40_specdata.npz',spfull = spfull,xfull = xfull,spnocor = spnocor,xnocor=xnocor)
